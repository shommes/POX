...
class l3_switch (EventMixin):
    def _handle_FlowRemoved(self, event): #GT 
        #Update flow entry in database with flow remove statistics #GT 
        self.dbHandler.updateFlowRemove(event.dpid, event.ofp)#GT 
        ...
        if isinstance(packet.next, ipv4):
            ...
            if dstaddr in self.arpTable[dpid]:
                ...
                if prt == inport:
                    ...
                    #Change flag to receive flow_remove event  #GT 
                    msg.flags = 1 #GT 
...
class DBHandler: #GT
    ...
    def addSwich(self, dpid):
        ...
        c2=self.r_server.hget(swStr, "removed")
        if (c==None):
            self.r_server.hmset(swStr,{"total":0, "removed":0})
        else:
            self.r_server.hmset(swStr,{"total":c, "removed":c2})
    
    '''Modify entry for switch when new flow is installed'''
    def modifySwichEntry(self, dpid, flowId):
        swStr="sw"+config.m[dpid]
        self.r_server.hincrby(swStr, "total")
        self.r_server.hset(swStr,flowId, "a")
    ...
    '''Update flow entry with statistics when flow was removed'''    
    def updateFlowRemove(self, dpid, flow):
        flowId=flow.cookie
        swStr="sw"+config.m[dpid]
        
        #mark flow as removed 
        self.r_server.hincrby(swStr, "removed")
        self.r_server.hset(swStr,flowId, "r")
        logger.info(' Mark removed flow with cookie = %s, for switch IP 10.92.0.%s ', flowId, config.m[dpid])
        
        #add flow remove counters to db
        self.r_server.hmset(flowId,  {"rp":flow.packet_count,
        "rb":flow.byte_count, 
        "ds":flow.duration_sec, 
        "dns":flow.duration_nsec, 
        "reason":flow.reason})
